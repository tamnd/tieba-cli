...
describes what users get. Pick a format with `-o`, or let tieba choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
tieba <command> -o table   # aligned columns for reading
tieba <command> -o jsonl   # one JSON object per line, for piping
tieba <command> -o json    # a single JSON array
tieba <command> -o csv     # spreadsheet friendly
tieba <command> -o tsv     # tab-separated
tieba <command> -o url     # just the URL column
tieba <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
tieba <command> --fields id,title,url
```
...
```bash
tieba <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
tieba <command>            # a table, because this is a terminal
tieba <command> | wc -l    # JSONL, because this is a pipe
```
