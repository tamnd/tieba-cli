// Package cli builds the tieba command tree on top of the tieba library.
package cli
...
import (
	...
	"github.com/tamnd/tieba-cli/pkg/render"
...
func Root() *cobra.Command {
	app := &App{cfg: tieba.DefaultConfig()}

	root := &cobra.Command{
		Use:   "tieba",
		Short: "Browse Baidu Tieba hot topics (百度贴吧热议)",
		Long: `tieba reads Baidu Tieba through its public mobile API.
No API key is required. It returns records as table, JSON, JSONL,
CSV, TSV, or URLs.

tieba is an independent tool and is not affiliated with Baidu.`,
		SilenceUsage:  true,
		SilenceErrors: true,
...
func (a *App) setup() error {
	...
	a.client = tieba.NewClient(a.cfg)
	return nil
}

func (a *App) renderW(w io.Writer, records any) error {
	r := render.New(w, render.Format(a.output), a.fields, a.noHeader, a.template)
	return r.Render(records)
}

func (a *App) render(records any) error {
	return a.renderW(os.Stdout, records)
}
